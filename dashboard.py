...
# ══════════════════════════════════════════════════════════════════════════
# РЕАЛЬНІ ДАНІ З ЗВІТУ
# ══════════════════════════════════════════════════════════════════════════

# Місячні дані (won/lost) — reconstructed to match report highlights
...
# CRM дані — з документа (точні цифри)
crm_df = pd.DataFrame({
...
overall_wr = round(total_won / total_all * 100, 1)
...
with col_l:
    ...
    fig_monthly.add_trace(go.Bar(
        x=monthly_df["Місяць"], y=monthly_df["Виграно"],
        name="Виграно ✅", marker_color="#27ae60",
    ))
    fig_monthly.add_trace(go.Bar(
        x=monthly_df["Місяць"], y=monthly_df["Програно"],
...
with col_r:
    ...
    st.dataframe(
        monthly_df[["Місяць","Виграно","Програно","Win Rate (%)"]].style
            .background_gradient(subset=["Win Rate (%)"], cmap="RdYlGn")
            .background_gradient(subset=["Виграно"], cmap="Greens")
            .format({"Win Rate (%)": "{:.1f}%"}),
        use_container_width=True, height=330,
    )

# ══════════════════════════════════════════════════════════════════════════
# РЯДОК 2: CRM Win Rate бар + таблиця
# ══════════════════════════════════════════════════════════════════════════
st.markdown('<div class="section-title">🖥️ Win Rate за CRM клієнта</div>', unsafe_allow_html=True)

filtered_crm = crm_df[crm_df["Всього"] >= min_deals].copy()
if not show_no_crm:
    filtered_crm = filtered_crm[filtered_crm["CRM"] != "No CRM"]

col_chart, col_table = st.columns([1.4, 1])

with col_chart:
    colors = ["#27ae60" if wr >= 50 else "#f39c12" if wr >= 30 else "#e74c3c"
              for wr in filtered_crm["Win Rate (%)"]]
    fig_crm = go.Figure(go.Bar(
        x=filtered_crm["Win Rate (%)"],
        y=filtered_crm["CRM"],
        orientation="h",
        marker_color=colors,
        text=[f"{v:.1f}%" for v in filtered_crm["Win Rate (%)"]],
        textposition="outside",
        hovertemplate="<b>%{y}</b><br>Win Rate: %{x:.1f}%<extra></extra>",
    ))
    fig_crm.add_vline(x=50, line_dash="dash", line_color="#555",
...
with col_table:
    st.dataframe(
        filtered_crm[["CRM","Виграно","Програно","Всього","Win Rate (%)"]].style
            .background_gradient(subset=["Win Rate (%)"], cmap="RdYlGn", vmin=0, vmax=70)
            .background_gradient(subset=["Всього"], cmap="Blues")
            .format({"Win Rate (%)": "{:.2f}%"}),
        use_container_width=True, height=420,
    )

# ══════════════════════════════════════════════════════════════════════════
# РЯДОК 3: Scatter + Donut
# ══════════════════════════════════════════════════════════════════════════
...
with col_s:
    ...
    fig_sc = px.scatter(
        filtered_crm, x="Всього", y="Win Rate (%)",
        size="Виграно", color="Win Rate (%)",
        text="CRM", color_continuous_scale="RdYlGn",
        range_color=[0, 70], height=320,
...
with col_p:
    ...
    fig_pie = px.pie(
        filtered_crm.nlargest(8, "Виграно"),
        values="Виграно", names="CRM",
        hole=0.4, height=320,
